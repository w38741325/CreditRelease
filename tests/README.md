...
Core tests take longer than any other Credit tests, due to the high amount of computational work involved in validating core components.
...
To run only Credit's core tests (after building):
...
To run only Credit's crypto tests (after building):
...
```bash
creditd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `credit-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
Credit is not currently registered as a separate OSS-Fuzz project. If you want to wire it up, start from the inherited upstream Monero integration
available [here](https://github.com/google/oss-fuzz/tree/master/projects/monero). OSS-Fuzz builds
and runs the fuzzers continuously, so long as the upstream Monero [build script](https://github.com/google/oss-fuzz/blob/master/projects/monero/build.sh) builds them.
...
**Building the inherited upstream Monero fuzzers with OSS-Fuzz**
...
To run only Credit's hash tests (after building):
...
To run only Credit's performance tests (after building):
...
To run only Credit's unit tests (after building):
