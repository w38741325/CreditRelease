# Anonymity Networks with Credit

Credit inherits Tor and I2P support from upstream Monero. The usage of
...
When the `--proxy` flag is used, `creditd` sends all external traffic via the
...
is made or until `creditd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `credit-wallet-cli` and
`credit-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
creditd does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a SOCKS4 proxy. creditd will connect to IPv4
nodes using this proxy to sync the blockchain.

```bash
creditd --proxy 127.0.0.1:9050 --p2p-bind-ip 127.0.0.1 --no-igd
```
...
`--tx-proxy` which tells `creditd` the ip/port of a socks proxy provided by a
...
which tells `creditd` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `creditd` the inbound address, network
...
which tells `creditd` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `creditd`
...
forwarded to `creditd` localhost port 18085. Using `tx-proxy`(required), these
...
`creditd` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/credit-rpc
...
```text
# P2P Hidden service
HiddenServiceDir /var/lib/tor/data/credit
...
This will store key information in `/var/lib/tor/data/credit` and `/var/lib/tor/data/credit-rpc`
and will forward "Tor port" 18084 and 18089 to ports 18084 and 18089 of ip 127.0.0.1, respectively. The file
`/var/lib/tor/data/credit/hostname` will contain the ".onion" address for use with `--anonymous-inbound`, and
`/var/lib/tor/data/credit-rpc/hostname` will contain the ".onion" address for use with RPC.
...
Please check the [upstream `monerod` reference](https://docs.getmonero.org/interacting/monerod-reference/#tori2p-and-proxies) for more information about these flags.
...
creditd will connect to IPv4 nodes via clearnet, revealing to your ISP
that you are running a Credit node, but your transactions will be relayed over
Tor.

```bash
sudo apt install tor # Or install Tor some other way
systemctl start tor # Or start Tor manually
creditd --tx-proxy tor,127.0.0.1:9050,10
```

### Connect To IPv4 Nodes Over Tor Only

This configuration does not connect to hidden services or accept incoming
connections. Your ISP will see that you are running Tor, but not creditd.
...
This configuration is not currently supported. creditd relies on IPv4 to sync
...
system clock is fairly accurate - many peers on the Credit network should have
similar timestamps.
...
### Intermittent Credit Syncing

If a user only runs `creditd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `creditd` as often as possible to conceal when transactions are being sent.
...
`creditd` currently selects two outgoing connections every 5 minutes for
...
`creditd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `creditd` should likely disconnect
