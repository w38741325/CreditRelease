# The Current/Future Status of ZMQ in Credit

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Credit
...
matching supported by both Credit and ZMQ. The `format`, `context` and `event`
...
The Credit daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Credit daemon also ensures that
...
The Credit daemon will send a `txpool_add` pub exactly once for each
